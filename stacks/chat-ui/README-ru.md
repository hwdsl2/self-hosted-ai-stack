...
> [!NOTE]
> При предоставлении доступа к AnythingLLM за пределами `localhost` или доверенной локальной сети используйте включённый Caddy HTTPS overlay, чтобы пароль шифровался при передаче, а прямые HTTP-порты были привязаны к localhost. См. ниже [Использование обратного прокси](#использование-обратного-прокси).
...
**Совет:** Готовые подстеки по умолчанию используют общие имена контейнеров, порты и Docker volumes. Если вам нужен один чат-интерфейс с голосом, RAG или инструментами, используйте полный стек или настройте Compose project names, имена контейнеров, порты и volumes перед параллельным запуском нескольких вариантов.
...
> [!IMPORTANT]
> AnythingLLM включает встроенную систему аутентификации пользователей — при открытии сервиса в интернет установите надёжный пароль при первоначальной настройке.
